...
export class OpenAIProvider implements AIProvider {
  ...
  readonly models: string[] = ['gpt-4o', 'gpt-4-turbo', 'gpt-3.5-turbo'];
  ...
  async generateCompletion(request: CompletionRequest): Promise<CompletionResponse> {
    ...
    try {
      ...
      logger.error({ err: error, provider: this.name, model }, 'OpenAI completion failed');
      throw new ProviderError(
  ...
  async generateStreaming(
    ...
    try {
      ...
      logger.error({ err: error, provider: this.name, model }, 'OpenAI streaming failed');
      await onChunk({ type: 'error', error: error.message ?? 'OpenAI streaming failed' });
      throw new ProviderError(
