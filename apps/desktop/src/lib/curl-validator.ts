...
function curlToJson(curl: string): any {
    const result: any = { headers: {}, method: 'GET' };
    const parts = parseCurlCommand(curl);
    
    for (const part of parts) {
        const trimmed = part.trim();
        if (trimmed.startsWith('curl ') || trimmed === 'curl') continue;
...
function parseCurlCommand(curl: string): string[] {
    const parts: string[] = [];
    let current = '';
    let inQuote = false;
    let quoteChar = '';
...
function extractHeaderValue(trimmed: string): string {
    const colonIdx = trimmed.indexOf(':');
    if (colonIdx === -1) return '';
    return trimmed.slice(colonIdx + 1).trim();
}

function extractMethodValue(trimmed: string): string {
    const spaceIdx = trimmed.indexOf(' ');
    return spaceIdx !== -1 ? trimmed.slice(spaceIdx + 1).trim().toUpperCase() : 'POST';
}

function extractDataValue(trimmed: string): string {
    const spaceIdx = trimmed.indexOf(' ');
    return spaceIdx !== -1 ? trimmed.slice(spaceIdx + 1).trim() : '';
}

export function parseCurlVariables(curlCommand: string): {
    url: string;
